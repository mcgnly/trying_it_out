...
def OneMillionMonkeys():
        #this is the text to match
    personText = input("What should the monkeys type for you today? ")
    #this generates an empty list of arbitrary length, because strings are immutable
    #Lists grow as necessary, you just cant access more than the current length
    monkeyText = []
    ...
    #populates the empty list with random characters, _ shows we don't care about which we are on, just do it to all
    #this is the same as for i in range(length) cause it just does it that many times anyway, not TO personText
    for _ in personText:
